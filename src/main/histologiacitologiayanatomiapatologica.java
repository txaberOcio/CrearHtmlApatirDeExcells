...
import clases.Cito;
import clases.CitoClass;
import clases.RegistroLibro;

public class histologiacitologiayanatomiapatologica {

	public static void main(String[] args) {
		
		
		String ruta="C:\\casetes\\histol.xls";
		   Cito reg= new Cito();
		   CitoClass obj=new CitoClass();	
		  ...
		  do{
		   if(cont==0){
			     ...
			     try{
			    	 obj=new CitoClass();
			    	 row=(HSSFRow) rows.next();
			    	 obj.setTitu(row.getCell(0).getStringCellValue());
			    	 obj.setDesc(row.getCell(1).getStringCellValue());
			    	 ...
			    	 try {
			    		 ...
			    		 obj.setUnidades(row.getCell(3).getStringCellValue());
			    	 }
			    	 try {
			    		 int num2=(int) row.getCell(4).getNumericCellValue();
			    		 obj.setFoto(String.valueOf(num2));
			    		 System.out.println(num2);
			    	 }catch(Exception e) {
			    		 obj.setFoto(row.getCell(4).getStringCellValue());
			    	 }
...
//enlace=nombre codigo=codigo descripcion= peso
	...
	for(int h=0;h<reg.getLista().size();h++) {
		...
		System.out.println(reg.getLista().get(h).getFoto());
		System.out.println(reg.getLista().get(h).getTitu());
		System.out.println(reg.getLista().get(h).getUnidades());
	...
	public static void CrearHtml(Cito listaRegistrosDefinitiva){
		FileWriter fichero = null;
		PrintWriter pw = null;
		String cod, chapter, subchapter,nombre;
		
			try {
				fichero = new FileWriter("C:\\Users\\PQNSV2\\Desktop\\histologiacitologiayanatomiapatologica.jsp");
				...
				pw.println("<div class=\"container\">\r\n <div class=\"row\">\r\n");
					...
					for(int e=0;e<listaRegistrosDefinitiva.getLista().size();e++){	
						pw.println("<div class='col-xs-12 col-sm-12 col-md-12 col-lg-12 col-xl-12' id='divproducto'>"+
								"<h1 class='prodtitulo2'>"+listaRegistrosDefinitiva.getLista().get(e).getTitu() +"</h1></div></div></div>");
								//fin titulo
								//contenido
								pw.println("<div class=\"container\">	\r\n <div class=\"row\" id=\"linea\">\r\n" +
										...
										+listaRegistrosDefinitiva.getLista().get(e).getDesc()+ 
										"</div>\r\n" + 
										"</div>\r\n" +
										"<div class=\"col-xs-12 col-sm-12 col-md-3 col-lg-3 col-xl-4\">\r\n"+
										"<img src='fotoproducto/"+(e+1)+".jpg' class='fotodelproducto'></div>"+
										"</div>\r\n" + 
										"</div>\r\n");						
					...
					"<div class='tabla'><table><tbody><tr class=''>"+
						"<th class='th1'>Codigo</th><th class='th1'>Unidades</th><th class='th1'>Precio</th><th class='th1'>Comprar</th></tr>");
						
				pw.println("<tr class='inpar'><td class='produccel'>"+listaRegistrosDefinitiva.getLista().get(e).getCodigo()+"</td><td class='produccel'>"+listaRegistrosDefinitiva.getLista().get(e).getUnidades()+
							"</td><td>"+
							"<div id='divCarga"+e+"' name='divCarga"+e+"'> </div>"+
							"<div id='borrar"+e+"' class='consultapre' onclick=\"cargarExterno3(`divCarga"+e+"`,'CEL"+listaRegistrosDefinitiva.getLista().get(e).getCodigo() +"','borrar"+e+"')\">Consultar precio</div></td>"+
							"<td class='comprar'><form name='comprar' action='/es/pedido/tramitaPedido.jsp' method='post'>"+
							"<input type='hidden' name='numLineas' value='1'><input type='hidden' name='linea1' value='CEL"+listaRegistrosDefinitiva.getLista().get(e).getCodigo()+"'>"+
