...
public class HTMLMerck {

	public static void main(String[] args) {
		
		
		String ruta="C:\\casetes\\merck1.xls";
		   Merck reg= new Merck();
		   ...
		   if(cont==0){
			     ...
			     try{
			    	 ...
			    	 try {
			    		 ...
			    		 obj.setTiempo(String.valueOf(num));
			    	 }catch(Exception e) {
			    		 
			    		 try{
			    			 obj.setTiempo(row.getCell(3).getStringCellValue());
			    		 }catch(Exception a) { 
			    		 }
			    	 }
			    	 try {
			    		 ...
			    		 try{
			    			 obj.setUnidades(row.getCell(4).getStringCellValue());
			    		 }catch(Exception a) { 
			    		 }
			    	 }
...
//enlace=nombre codigo=codigo descripcion= peso
	...
	for(int h=0;h<reg.getLista().size();h++) {
		...
		System.out.println(reg.getLista().get(h).getTiempo());
	...
	public static void CrearHtml(Merck listaRegistrosDefinitiva){
		...
		String cod, chapter, subchapter,nombre;
			...
			try {
				fichero = new FileWriter("C:\\Users\\PQNSV2\\Desktop\\PCRbiologiamolecular.jsp");
				...
				pw.println("<div class=\"container\">\r\n <div class=\"row\">\r\n");
					...
					for(int e=0;e<listaRegistrosDefinitiva.getLista().size();e++){	
						if(listaRegistrosDefinitiva.getLista().get(e).getDesc()==null){
							...
							pw.println("<tr class=''><th class='th1'>Codigo</th><th class='th1'>Nombre</th><th class='th1'>descripcion</th><th class='th1'>Tiempo</th><th class='th1'>Unidades</th><th class='th1'>Precio</th><th class='th1'>Comprar</th></tr>");
				...
				pw.println("<td class='produccel'>"+listaRegistrosDefinitiva.getLista().get(e).getDesc()+"</td><td class='produccel'>"+listaRegistrosDefinitiva.getLista().get(e).getTiempo()+"</td>");
				pw.println("<td class='produccel'>"+listaRegistrosDefinitiva.getLista().get(e).getUnidades()+"</td>");
							pw.println("<td class='produccel'><div id='divCarga"+e+"' name='divCarga"+e+"'> </div>"+
							"<div id='borrar"+e+"' class='consultapre' onclick=\"cargarExterno3(`divCarga"+e+"`,'ME"+listaRegistrosDefinitiva.getLista().get(e).getCodigo() +"','borrar"+e+"')\">Consultar precio</div></td>"+
							"<td class='comprar'><form name='comprar' action='/es/pedido/tramitaPedido.jsp' method='post'>"+
							"<input type='hidden' name='numLineas' value='1'><input type='hidden' name='linea1' value='ME"+listaRegistrosDefinitiva.getLista().get(e).getCodigo()+"'>"+
